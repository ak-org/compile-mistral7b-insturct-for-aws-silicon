...
#os.environ['NEURON_RT_NUM_CORES'] = '12'
...
class LLMHandler(BaseHandler, ABC):
    ...
    def initialize(self, ctx):
        ...
        if 'NEURON_CORES' in os.environ:
            self.tp_degree = int(os.environ['NEURON_CORES'])
        else:
            self.tp_degree = ctx.model_yaml_config.get("handler", {}).get("tp_degree", 2)
        ...
        model_compiled_path = f"{model_dir}/{model_compiled_dir}-{self.tp_degree}"
        ...
        self.amp = ctx.model_yaml_config.get("handler", {}).get("amp", "bf16")
