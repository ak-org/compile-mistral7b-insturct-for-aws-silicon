...
def llama2_7b_chat_inference_calls(prompt, endpoint_name):
    body = {
            "inputs": f"""{prompt} \n Response:""",
            "parameters": {'n_positions': 1024, 'top_p': 0.9, 'temperature': 0.9}
        }
...
def generate_results(model_id, endpoint_name, tokenizer, prompt):
    if model_id == 'mistral':
        duration, input_prompt, results = mistral_inference_calls(prompt, endpoint_name=endpoint_name)
    elif model_id == 'llama2':
        duration, input_prompt, results = llama2_7b_chat_inference_calls(prompt, endpoint_name=endpoint_name)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print('Need two paramters: <sm endpoint name> <mistral|llama2>')
        exit(-1)
    boto_config = Config(
    ...
    for prompt_file in ['short_prompts.txt', 'medium_prompts.txt']:
        with open(prompt_file, 'r')as f:
            ...
            for prompt in prompts:
                ...
                results, \
                input_tokens_count, \
                results_tokens_count = generate_results(model_id,
                                                        endpoint_name, 
                                                        tokenizer, 
                                                        prompt.replace('\n', ''))
